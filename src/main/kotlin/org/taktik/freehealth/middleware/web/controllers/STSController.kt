...
class STSController(private val stsService: STSService, private val ssoService: SSOService) {
    ...
    @Operation(
        ...
        description = "Uploads a keystore file to the STS service and returns the UUID of the uploaded keystore. The kestore file must be in PKCS12 format and should contain the private key and certificate chain for the user. It will never be stored anywhere but encrypted in RAM"
    )
    ...
    @Operation(
        ...
        description = "Retrieves a SAML token for the user identified by the given keystoreId and passPhrase. The tokenId is used to identify the token. This endpoint is useful for obtaining a SAML token that can be used for authentication and authorization in other systems."
    )
    ...
    @Operation(
        ...
        description = "Check that a cached SAML token is still valid. The tokenId is used to identify the token. This endpoint is useful for verifying the validity of a cached SAML token before using it for authentication or authorization."
    )
