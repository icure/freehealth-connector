...
public final class GenericRequest {
   ...
   public GenericRequest addDefaulHandlerChain() throws TechnicalConnectorException {
      for(SOAPHandler handler : (new ConfigurableFactoryHelper("connector.defaulthandlerchain.beforesecurity", (String)null)).getImplementations()) {
         this.handlerChain.register(HandlerPosition.BEFORE, handler);
      }

      for(SOAPHandler handler : (new ConfigurableFactoryHelper("connector.defaulthandlerchain.aftersecurity", (String)null)).getImplementations()) {
