...
public final class CacheFactory {
   ...
   public static <K, V> Cache<K, V> newInstance(CacheType cacheType, String cacheName, CacheInformation.ExpiryType expiryType, Duration expiryDuration) {
      try {
         ConfigurableFactoryHelper<Cache<K, V>> helper;
         switch (cacheType) {
            case MEMORY:
               helper = new ConfigurableFactoryHelper<Cache<K, V>>("be.ehealth.technicalconnector.cache.memory.impl", DEFAULT_CACHE_MEMORY_IMPL);
               break;
            case PERSISTENT:
               helper = new ConfigurableFactoryHelper<Cache<K, V>>("be.ehealth.technicalconnector.cache.persistent.impl", DEFAULT_CACHE_PERSISTENT_IMPL);
               break;
            default:
         ...
         return helper.getImplementation(options);
