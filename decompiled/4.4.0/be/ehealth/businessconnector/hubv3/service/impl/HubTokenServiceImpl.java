...
public class HubTokenServiceImpl implements HubTokenService, ConfigurationModuleBootstrap.ModuleBootstrapHook {
   public static final String PROP_INTRAHUBV3_SHA1FEATURE_ENABLED = "intrahubv3.sha1feature.enabled";
   ...
   private <T> T executeOperation(SAMLToken token, Object request, String operation, Class<T> clazz) throws TechnicalConnectorException {
      try {
         GenericRequest service = null;
         if (ConfigFactory.getConfigValidator().getBooleanProperty("intrahubv3.sha1feature.enabled", false)) {
            service = ServiceFactory.getIntraHubPort(token, operation).setPayload(request, new SHA1Feature());
         } else {
            service = ServiceFactory.getIntraHubPort(token, operation).setPayload(request);
         }
