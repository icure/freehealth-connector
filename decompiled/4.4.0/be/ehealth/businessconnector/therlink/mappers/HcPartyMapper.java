...
public class HcPartyMapper {
   ...
   private static Collection<? extends IDHCPARTY> mapIdsAndPropertiesToIdList(HcParty hcp) {
      ...
      if (hcp.getInss() != null) {
         IDHCPARTY idInss = HcPartyUtil.buildId("1.0", hcp.getInss(), IDHCPARTYschemes.INSS, (String)null);
         if (!verifyListIfIDExists(result, idInss)) {
            result.add(idInss);
         }
      }

      if (hcp.getNihii() != null) {
         IDHCPARTY idNihii = HcPartyUtil.buildId("1.0", hcp.getNihii(), IDHCPARTYschemes.ID_HCPARTY, (String)null);
         if (!verifyListIfIDExists(result, idNihii)) {
            result.add(idNihii);
         }
      }

      if (hcp.getCbe() != null) {
         IDHCPARTY idCbe = HcPartyUtil.buildId("1.0", hcp.getCbe(), IDHCPARTYschemes.ID_HCPARTY, (String)null);
         if (!verifyListIfIDExists(result, idCbe)) {
            result.add(idCbe);
         }
      }

      if (hcp.getApplicationID() != null) {
         IDHCPARTY idApplication = HcPartyUtil.buildId("1.0", hcp.getApplicationID(), IDHCPARTYschemes.LOCAL, "application_ID");
         if (!verifyListIfIDExists(result, idApplication)) {
            result.add(idApplication);
         }
      }

      if (hcp.getEHP() != null) {
         IDHCPARTY ehp = HcPartyUtil.buildId("1.0", hcp.getEHP(), IDHCPARTYschemes.ID_HCPARTY, (String)null);
   ...
   private static boolean verifyListIfIDExists(List<IDHCPARTY> ids, IDHCPARTY idToVerify) {
      for(IDHCPARTY existingId : ids) {
         if (areIdsTheSame(idToVerify, existingId)) {
            return true;
         }
      }
