   ...
   public final LocalizedText asLocalizedText(TextType inTextType) {
      ...
      if (inTextType != null) {
         ...
         if (inTextType.getValues() != null) {
            ...
            for(be.fgov.ehealth.certra.core.v2.LocalizedString avaluesItem : inTextType.getValues()) {
   ...
   public final TextType asTextType(LocalizedText inLocalizedText) {
      ...
      if (inLocalizedText != null) {
         ...
         if (inLocalizedText.getValues() != null) {
            ...
            for(LocalizedString avaluesItem : inLocalizedText.getValues()) {
               avaluesTmpCollection.add(this.asLocalizedString(avaluesItem));
            }
         }
      }
   ...
   public final ActorType asActorType(Actor inActor) {
      ...
      if (inActor != null) {
         ...
         if (inActor.getFirstNames() != null) {
            List afirstnamesTmpCollection = out.getFirstNames();

            for(String afirstnamesItem : inActor.getFirstNames()) {
               afirstnamesTmpCollection.add(afirstnamesItem);
            }
         }

         if (inActor.getIds() != null) {
            List aidsTmpCollection = out.getIds();

            for(ActorId aidsItem : inActor.getIds()) {
   ...
   public final Actor asActor(ActorType inActorType) {
      ...
      if (inActorType != null) {
         ...
         if (inActorType.getFirstNames() == null) {
            ...
            ArrayList<String> afirstnamesTmpCollection = new ArrayList(inActorType.getFirstNames().size());

            for(String afirstnamesItem : inActorType.getFirstNames()) {
               afirstnamesTmpCollection.add(afirstnamesItem);
            }

            out.setFirstNames(afirstnamesTmpCollection);
         }

         if (inActorType.getIds() == null) {
            out.setIds((List)null);
         } else {
            ArrayList<ActorId> aidsTmpCollection = new ArrayList(inActorType.getIds().size());

            for(Id aidsItem : inActorType.getIds()) {
   ...
   public final ActorQualities asActorQualities(GetActorQualitiesResponse inGetActorQualitiesResponse) {
      ...
      if (inGetActorQualitiesResponse != null) {
         ...
         if (inGetActorQualitiesResponse.getOrganizationAuthorizations() != null) {
            ...
            for(OrganizationType aorganizationauthorizationsItem : inGetActorQualitiesResponse.getOrganizationAuthorizations()) {
