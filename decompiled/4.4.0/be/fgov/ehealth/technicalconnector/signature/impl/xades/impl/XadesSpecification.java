...
public class XadesSpecification implements be.fgov.ehealth.technicalconnector.signature.impl.xades.XadesSpecification {
   ...
   private void verifyDigest(SignatureVerificationResult result, Element certEl) {
      ...
      try {
         ...
         LOG.warn("Unable to encode certificate with CN [{}] Reason: {}", new Object[]{signingCert.getSubjectX500Principal().getName("RFC1779"), e.getMessage(), e});
         result.getErrors().add(SignatureVerificationError.XADES_SIGNEDPROPS_COULD_NOT_BE_VERIFIED);
      } catch (NoSuchAlgorithmException e) {
         LOG.error("Invalid digest method [{}]", digestMethod, e);
         result.getErrors().add(SignatureVerificationError.XADES_SIGNEDPROPS_NOT_VALID);
      }
