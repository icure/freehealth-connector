...
public class ResponseBuilderImpl implements ResponseBuilder, ConfigurationModuleBootstrap.ModuleBootstrapHook {
   ...
   private ChapterIVKmehrResponseWithTimeStampInfo validateTimestampAndretrieveChapterIVKmehrResponseWithTimeStampInfo(Chap4MedicalAdvisorAgreementResponseWrapper<?> agreementResponse, ConversationType responseType, boolean ignoreWarnings) throws ChapterIVBusinessConnectorException, UnsealConnectorException, TechnicalConnectorException {
      ...
      this.validateTimeStamp(unsealedResponse.getKmehrResponseBytes(), unsealedResponse.getTimestampReplyBytes());
   ...
   private void validateTimeStamp(byte[] kmehrResponse, byte[] timestampReplyBytes) throws TechnicalConnectorException {
      TimeStampToken timeStampToken = null;

      try {
         timeStampToken = TimestampUtil.getTimestampResponse(timestampReplyBytes).getTimeStampToken();
      } catch (Exception var5) {
         timeStampToken = TimestampUtil.getTimeStampToken(timestampReplyBytes);
      }

      TimeStampValidatorFactory.getInstance().validateTimeStampToken(kmehrResponse, timeStampToken);
   }
