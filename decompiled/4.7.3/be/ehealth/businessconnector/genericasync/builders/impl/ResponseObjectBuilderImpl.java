...
public class ResponseObjectBuilderImpl implements ResponseObjectBuilder {
   ...
   public <T> ProcessedGetResponse processResponse(GetResponse getResponse, Class<T> clazz, String projectName, String schemaLocation) throws GenAsyncBusinessConnectorException, TechnicalConnectorException {
      ...
      for(MsgResponse msgResponse : getResponse.getReturn().getMsgResponses()) {
         ...
         if (msgResponse.getDetail().getContentEncryption() == null) {
            ...
            processedMsgResponse = new ProcessedMsgResponse<T>(msgResponse, this.toBusinessResponse(clazz, unwrappedMessageByteArray), this.validateMsgXadesT(msgResponse, msgResponse.getXadesT().getValue(), projectName), ArrayUtils.isEmpty(msgResponse.getXadesT().getValue()) ? null : unwrappedMessageByteArray);
         } else {
            processedMsgResponse = this.<T>processEncryptedResponse(msgResponse, projectName, clazz);
         }
   ...
   public <T> ProcessedMsgResponse<T> processEncryptedResponse(MsgResponse msgResponse, String projectName, Class<T> clazz) throws TechnicalConnectorException, GenAsyncBusinessConnectorException {
      ...
      ProcessedMsgResponse<T> processedMsgResponse = new ProcessedMsgResponse<T>(msgResponse, this.toBusinessResponse(clazz, decryptedMessageByteArray), encryptedKnownContent, this.validateMsgXadesT(encryptedKnownContent, encryptedKnownContent.getXades(), projectName), ArrayUtils.isEmpty(encryptedKnownContent.getXades()) ? null : unsealedData);
      return processedMsgResponse;
   }

   private <T> T toBusinessResponse(Class<T> clazz, byte[] unwrappedMessageByteArray) {
      return (T)(clazz.equals(byte[].class) ? unwrappedMessageByteArray : ConnectorXmlUtils.toObject(unwrappedMessageByteArray, clazz));
   }
   ...
   private SignatureVerificationResult validateXadesT(Object value, byte[] xadesT, boolean followNestedManifest) throws GenAsyncBusinessConnectorException {
      if (!ArrayUtils.isEmpty(xadesT)) {
         ...
         try {
            ...
            throw new GenAsyncBusinessConnectorException(GenAsyncBusinessConnectorExceptionValues.SIGNATURE_VALIDATION_ERROR, e, new Object[]{e.getMessage()});
         }
      } else {
         return null;
      }
