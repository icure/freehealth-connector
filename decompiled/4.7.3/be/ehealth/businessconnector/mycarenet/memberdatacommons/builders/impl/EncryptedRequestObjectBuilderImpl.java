...
public class EncryptedRequestObjectBuilderImpl implements RequestObjectBuilder, ConfigurationModuleBootstrap.ModuleBootstrapHook {
   ...
   private String projectIdentifier = "memberdata";

   public EncryptedRequestObjectBuilderImpl() {
   }

   public EncryptedRequestObjectBuilderImpl(String projectIdentifier) {
   ...
   public MemberDataConsultationRequest buildConsultationRequest(boolean isTest, InputReference references, Object attrQuery) throws TechnicalConnectorException, MemberDataBusinessConnectorException {
      ...
      BlobBuilder blobBuilder = BlobBuilderFactory.getBlobBuilder(this.projectIdentifier);
      ...
      try {
         attrQueryBytes = (new MemberDataEncryptionUtil()).handleEncryption(encryptedKnownContent, SessionUtil.getHolderOfKeyCrypto(), this.projectIdentifier);
      ...
      request.setCommonInput(((CommonInputMapper)Mappers.getMapper(CommonInputMapper.class)).map(commonBuilder.createCommonInput(McnConfigUtil.retrievePackageInfo(this.projectIdentifier), isTest, references.getInputReference())));
