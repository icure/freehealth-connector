...
public final class ValidatorHelper {
      ...
      try {
         ...
         for(int i = 0; i < schemaFiles.length; ++i) {
            ...
            if (schemaFiles.length == 1) {
               sources = (Source[])ArrayUtils.add(sources, new StreamSource(SchemaValidatorHandler.class.getResource(schemaFile).toExternalForm()));
            } else {
   ...
   private static void handleValidationResult(ErrorCollectorHandler collector) throws TechnicalConnectorException {
      if (collector.hasExceptions("WARN")) {
         for(String exception : collector.getExceptionList("WARN")) {
            LOG.warn(exception);
         }
      }
      ...
      if (collector.hasExceptions("ERROR", "FATAL")) {
         ...
         for(String exception : collector.getExceptionList("ERROR", "FATAL")) {
            LOG.error(exception);
            sb.append(exception);
            sb.append(", ");
         }
