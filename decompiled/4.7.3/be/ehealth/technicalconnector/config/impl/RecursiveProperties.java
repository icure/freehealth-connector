...
public class RecursiveProperties extends Properties {
   ...
   private static Map<String, Lookup> create(Lookup... lookups) {
      Map<String, Lookup> result = new RegexHashMap<String, Lookup>();
   ...
   private static class RegexHashMap<K, V> extends HashMap<K, V> {
      ...
      public V get(Object key) {
         for(Map.Entry<Pattern, V> matcher : this.matcherMap.entrySet()) {
            if (((Pattern)matcher.getKey()).matcher(key.toString()).matches()) {
      ...
      public String process(String value, List<String> lookupKeys) {
         ...
         if (value == null) {
            ...
            if (this.pattern.matcher(value).matches()) {
               for(String ref : new HashSet(Arrays.asList(StringUtils.substringsBetween(value, this.startTag, this.endTag)))) {
