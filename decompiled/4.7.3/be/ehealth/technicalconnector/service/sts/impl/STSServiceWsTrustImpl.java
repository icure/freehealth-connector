   ...
   private Element issueHokToken(Credential bodyCredentials, List<SAMLAttribute> attributes, List<SAMLAttributeDesignator> designators, Duration validity) throws TechnicalConnectorException {
      try {
         ...
         for(SAMLAttribute attr : attributes) {
            ...
            claimValue.setTextContent(StringUtils.join(attr.getValues(), ";"));
         ...
         lifetimeExpires.setTextContent(DateUtils.printDateTime(now.plusSeconds((int)validity.convert(TimeUnit.SECONDS))));
